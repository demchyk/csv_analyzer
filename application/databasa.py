...
class DataBasa:
    ...
    def result_to_pickle(self):
        ...
        new_pickle = (
            self.__replace_dtypes_in_dataframe(new_pickle).sort_values(self.__data_time_field_name).drop_duplicates().reset_index(drop=True)
        )  # convert to lighter dtypes
        try:
            print(ctime(), '--- Reading info from existing pickles..')
            new_pickle = (
                pd.concat([new_pickle, pd.read_pickle(self.__table_name, compression='zip')])
                .sort_values(self.__data_time_field_name)
                .drop_duplicates()
                .reset_index(drop=True)
            )  # if pickle already exists - concat in one pickle and drop duplicates
        except:
            pass
        finally:
            if not new_pickle.attrs:  # Create final pickle with attributes
                new_pickle.attrs = self.__get_attr_for_dataframe(
                    self.__ZTE_type,
                    self.__node_name,
                    self.__cell_name,
                    self.__data_time_field_name,
                    self.__metrics_names,
                    self.__metrics,
                    self.__primary_keys,
                    self.__counters,
                )
